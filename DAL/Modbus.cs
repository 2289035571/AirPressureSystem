...
namespace DAL
{
    public class Modbus
    {
        ...
        public bool OpenMyCom(int iBaudRate, string iPortNo, int iDataBits, Parity iParity, StopBits iStopBits)
        {
            ...
            {
                Console.WriteLine("报错信息" + ex.ToString());
                return false;
            }
        ...
        public byte[] ReadOutputStatus(int iDevAdd, int iAddress, int iLength)
        {
            ...
            sendCommand[1] = 0x01; 
            sendCommand[2] = (byte)((iAddress - iAddress % 256) / 256); //高位
            sendCommand[3] = (byte)(iAddress % 256); //低位
            sendCommand[4] = (byte)((iLength - iLength % 256) / 256);
            sendCommand[5] = (byte)(iLength % 256);
            Crc16(sendCommand, 6); //CRC校验 ，将得到的值赋给索引6 7
        ...
        /// <returns></returns>
        private byte[] GetByteArray(byte[] byteArray, int start, int length)
        {
            byte[] Res = new byte[length];
            if(byteArray != null && byteArray.Length > length) {
                for (int i = 0; i < length; i++)
                {
                    Res[i] = byteArray[i + start];
                }
